...
\title{{\textbf{\huge\textcolor{nicered}{DyNaMol}}} \\ \textsc{\small Tight-Binding Magnetic Molecular Dynamics for everyone}}
...
The first step is to download the latest release of DyNaMol from its github repository. To proceed, you have to check that git is installed~:
\begin{lstlisting}[language=bash,basicstyle=\small]
$ locate git
\end{lstlisting}
Then, clone the distant repository to your local one~:
\begin{lstlisting}[language=bash,basicstyle=\small]
$ git clone https://github.com/araven/DyNaMol.git
\end{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%
\section{Linux}
Preferred method : on Ubuntu 16.04 and later, install gfortran and cmake to compile dynamol. Be sure to get the mandatory related dependencies of these packages.
...
In order to build the code, to the root of dynamol directory~:
\begin{lstlisting}[language=sh,basicstyle=\small]
...
In order to build the code, open a MSYS MINGW 64-bit console. To the root of dynamol directory~:
\begin{lstlisting}[language=sh,basicstyle=\small]
...
$ mingw32-make
\end{lstlisting}
In order to run dynamol.exe, be sure you have the TERM and TERMINFO environment variables up to date into your .bashrc file~:
\begin{lstlisting}[language=sh,basicstyle=\small]
$ export TERM=xterm
